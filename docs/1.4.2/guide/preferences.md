...
[Project Properties](/1.4.2/guide/projects#project-properties)).
...
![The Preferences window with its tabs](/1.4.2/images/preferences.png)
...
- **Waveform display** — choose whether clips draw a **Single waveform** (one
  ...
  [Clip Editor](/1.4.2/guide/clip-editor).
- **Show images on library tiles** — show embedded cover art (or a fallback
  audio icon) on each [Library](/1.4.2/guide/library) tile. Turn it off for a denser,
  text-only library.
...
- **Auto-warp clips to project tempo** — when you drag a clip onto a track,
  automatically switch on [Warp](/1.4.2/guide/clip-editor#matching-tempo-warp) so the
...
- **Previous / next buttons** — choose where the transport's previous and next
  ...
  (step through your [timeline markers](/1.4.2/guide/timeline#markers)).
...
- **Autosave** — snapshot projects with unsaved changes in the background, so
  ...
  600 seconds). See
  [Autosave and crash recovery](/1.4.2/guide/projects#autosave-and-crash-recovery).
...
- **Default project sample rate** — the rate (**44.1 kHz** or **48 kHz**) new
  ...
  change in [Project Properties](/1.4.2/guide/projects#project-properties).
...
Set up supported [MIDI Controllers](/1.4.2/guide/midi-controllers):
...
[Brake and Backspin](/1.4.2/guide/clip-editor#turntable-effects-brake-and-backspin)):
...
These settings shape the [Scratch Editor](/1.4.2/guide/scratch-editor):
...
Defaults for [stem separation](/1.4.2/guide/stems). *Models* are the engines Silverdaw
