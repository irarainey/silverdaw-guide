...
- **Graphics card (optional):** a compatible GPU can speed up
  [stem separation](/1.0/guide/stems), but it is not required — Silverdaw can do the
...
[Quick Start](/1.0/guide/quick-start) to build your first remix, or take the
[tour of the Silverdaw window](/1.0/guide/the-window) first.

> If you do not hear anything when you press play, check that the right audio
> output is selected. See [Choosing your audio output](/1.0/guide/the-window#choosing-your-audio-output).
