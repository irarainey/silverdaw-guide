...
![The timeline with clips arranged across several tracks](/1.6/images/timeline-arranging.png)
...
- **Trim** — drag a clip's edge to shorten or extend how much of it plays. A
  [linked clip](/1.6/guide/library#linked-clips) can't be edge-trimmed on the
...
- **Rename** — double-click a clip's **title strip** (the thin band along its top)
  ...
  [Clip Editor](/1.6/guide/clip-editor) instead.
- **Open ▸ Scratch Editor** — start a scratch session using the clip. Saving the
  ...
  [Library](/1.6/guide/library), without replacing the clip. See
  [Scratch Editor](/1.6/guide/scratch-editor).
...
The **grid** control in the [status bar](/1.6/guide/the-window#the-status-bar) —
...
other. A [linked](/1.6/guide/library#linked-clips) or **locked** clip can't be
...
[export your mix](/1.6/guide/export).

## Bar numbering

The timeline is numbered in bars, starting at **1** by default. You can change
where the count starts in [Project Properties](/1.6/guide/projects#project-properties)
...
[Warp](/1.6/guide/clip-editor#matching-tempo-warp) for you). You can turn this
...
clip in the [Clip Editor](/1.6/guide/clip-editor) and use the **Beat grid**
...
[Fixing the detected tempo](/1.6/guide/clip-editor#fixing-the-detected-tempo).
...
[Preferences ▸ Timeline](/1.6/guide/preferences#timeline). On a
[MIDI controller](/1.6/guide/midi-controllers), the hot-cue pads jump to
...
[Automation](/1.6/guide/mixing#automation-changing-settings-over-time).
