...
![The timeline with clips arranged across several tracks](/1.3/images/timeline-arranging.png)
...
- **Trim** — drag a clip's edge to shorten or extend how much of it plays. A
  [linked clip](/1.3/guide/library#linked-clips) can't be edge-trimmed on the
...
- **Rename** — double-click a clip's **title strip** (the thin band along its top)
  ...
  [Clip Editor](/1.3/guide/clip-editor) instead.
- **Open ▸ Scratch Editor** — start a scratch session using the clip. Saving the
  ...
  [Library](/1.3/guide/library), without replacing the clip. See
  [Scratch Editor](/1.3/guide/scratch-editor).
...
other. A [linked](/1.3/guide/library#linked-clips) or **locked** clip can't be
...
[export your mix](/1.3/guide/export).

## Bar numbering

The timeline is numbered in bars, starting at **1** by default. You can change
where the count starts in [Project Properties](/1.3/guide/projects#project-properties)
...
- If detection is uncertain, you can set a clip's **BPM by hand** and slide the
  ...
  [Clip Editor](/1.3/guide/clip-editor#matching-tempo-warp).
- The project's own tempo is set in
  [Project Properties](/1.3/guide/projects#project-properties).

By default, dropping a musical clip onto a track automatically **matches it to
the project tempo** (Silverdaw switches on
[Warp](/1.3/guide/clip-editor#matching-tempo-warp) for you). You can turn this
...
[Preferences ▸ Timeline](/1.3/guide/preferences#timeline).
...
[Automation](/1.3/guide/mixing#automation-changing-settings-over-time).
