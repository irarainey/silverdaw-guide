...
![The Preferences window with its tabs](/1.8/images/preferences.png)
...
- **Waveform display** — choose whether clips draw a **Single waveform** (one
  ...
  [Clip Editor](/1.8/guide/clip-editor).
- **Show images on library tiles** — show embedded cover art (or a fallback
  audio icon) on each [Library](/1.8/guide/library) tile. Turn it off for a denser,
  text-only library.
...
- **Auto-warp clips to project tempo** — when you drag a clip onto a track,
  automatically switch on [Warp](/1.8/guide/clip-editor#matching-tempo-warp) so the
...
- **Previous / next buttons** — choose where the transport's previous and next
  ...
  (step through your [timeline markers](/1.8/guide/timeline#markers)).
...
- **Autosave** — snapshot projects with unsaved changes in the background, so
  ...
  600 seconds). See
  [Autosave and crash recovery](/1.8/guide/projects#autosave-and-crash-recovery).
...
- **Default project sample rate** — the rate (**44.1 kHz** or **48 kHz**) new
  ...
  change in [Project Properties](/1.8/guide/projects#project-properties).
...
Set up supported [MIDI Controllers](/1.8/guide/midi-controllers):
...
These settings shape the [Scratch Editor](/1.8/guide/scratch-editor):
...
Defaults for [stem separation](/1.8/guide/stems). *Models* are the engines Silverdaw
