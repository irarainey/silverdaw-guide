...
Clips on a track you cannot currently hear — because it is muted, or because
another track is soloed — are drawn faded on the timeline, so it is obvious at a
glance what is and isn't playing.
...
Beyond the built-in effects, you can add **VST3** effect plugins to a track from
the **Plugins** tab in the bottom panel. They run as a chain after Track FX and
before the track's volume, pan, and sends. See [Plugins](/1.9/guide/plugins).
...
- **Scratch realism** applies only while a platter is held in the
  [Scratch Editor](/1.9/guide/scratch-editor#scratch-realism). It softens the
...
Click the **A** button on a track header to show its automation lane, then draw a
**breakpoint curve** — a line with points you can shape. The first lane is
...
saved with the project. You can automate:

![A track with its automation lane open and a breakpoint curve](/1.9/images/mixing-automation.png)

<!-- Image placeholder: two visible automation lanes for different parameters on one track. -->
...
[change the project tempo](/1.9/guide/timeline#changing-the-project-tempo), every
