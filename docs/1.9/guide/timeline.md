...
![The timeline with clips arranged across several tracks](/1.9/images/timeline-arranging.png)
...
Windows Explorer, or a row from the [Files tab](/1.9/guide/files), onto a track
...
- **Trim** — drag a clip's edge to shorten or extend how much of it plays. A
  [linked clip](/1.9/guide/library#linked-clips) can't be edge-trimmed on the
...
- **Rename** — double-click a clip's **title strip** (the thin band along its top)
  ...
  [Clip Editor](/1.9/guide/clip-editor) instead.
- **Open ▸ Scratch Editor** — start a scratch session using the clip. Saving the
  ...
  [Library](/1.9/guide/library), without replacing the clip. See
  [Scratch Editor](/1.9/guide/scratch-editor).
...
The **grid** control in the [status bar](/1.9/guide/the-window#the-status-bar) —
...
other. A [linked](/1.9/guide/library#linked-clips) or **locked** clip can't be
...
[export your mix](/1.9/guide/export).

## Bar numbering

The timeline is numbered in bars, starting at **1** by default. You can change
where the count starts in [Project Properties](/1.9/guide/projects#project-properties)
...
- If detection is uncertain, you can set a clip's **BPM by hand** and slide the
  ...
  [Clip Editor](/1.9/guide/clip-editor#matching-tempo-warp), and is explained in
  ...
  with **Edit BPM…** — see
  [Correcting a tempo](/1.9/guide/library#correcting-a-tempo).
- The project's own tempo can be set from the transport bar or in
  [Project Properties](/1.9/guide/projects#project-properties), and can be
...
[Warp](/1.9/guide/clip-editor#matching-tempo-warp) for you). You can turn this
...
[Project Properties](/1.9/guide/projects#project-properties). Silverdaw retimes
...
- **Clip [volume shapes](/1.9/guide/clip-editor#shaping-a-clip-s-volume)** are
  resized whenever the clip they are drawn on is re-stretched, so each fade and
  dip stays over the same part of the sound.
- **[Track automation](/1.9/guide/mixing#automation-changing-settings-over-time)**
...
- A clip set to **Follow project BPM** in the
  [Clip Editor](/1.9/guide/clip-editor#matching-tempo-warp) always tracks the
  project tempo.
...
clip in the [Clip Editor](/1.9/guide/clip-editor) and use the **Beat grid**
...
[Preferences ▸ Timeline](/1.9/guide/preferences#timeline). On a
[MIDI controller](/1.9/guide/midi-controllers), the hot-cue pads jump to
...
[Automation](/1.9/guide/mixing#automation-changing-settings-over-time).
