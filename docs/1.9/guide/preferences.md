...
across the whole application (a few settings, like the audio output device, can also be
...
![The Preferences window with its tabs](/1.9/images/preferences.png)
...
- **Waveform display** — choose whether clips draw a **Single waveform** (one
  ...
  single waveform. Your choice applies everywhere a waveform is drawn: the
  timeline, the [Clip Editor](/1.9/guide/clip-editor), the
  [Scratch Editor](/1.9/guide/scratch-editor), and the
  [Record Audio](/1.9/guide/recording) dialog.
- **Show images on library tiles** — show embedded cover art (or a fallback
  audio icon) on each [Library](/1.9/guide/library) tile. Turn it off for a denser,
  text-only library.
...
- **Auto-warp clips to project tempo** — when you drag a clip onto a track,
  automatically switch on [Warp](/1.9/guide/clip-editor#matching-tempo-warp) so the
...
- **Previous / next buttons** — choose where the transport's previous and next
  ...
  (step through your [timeline markers](/1.9/guide/timeline#markers)).
...
- **Autosave** — snapshot projects with unsaved changes in the background, so
  ...
  600 seconds). See
  [Autosave and crash recovery](/1.9/guide/projects#autosave-and-crash-recovery).
- **Clean up project files on remove** — when you remove a stem, sample, or recording from
...
Where and how Silverdaw plays and records sound:
...
- **Default project sample rate** — the rate (**44.1 kHz** or **48 kHz**) new
  ...
  change in [Project Properties](/1.9/guide/projects#project-properties).
...
- **Audio output driver** — an optional, advanced choice of the audio system
  Silverdaw uses for the selected output device. Each choice is summarised in a
  few words. The default suits most people.
- **Recording input driver** — which driver
  [Record Audio](/1.9/guide/recording) takes microphones and interfaces from.
  Windows offers the same input through several drivers, so leave this on
  **Automatic — pick the best available** unless your input is missing from the
  list or sounds wrong. The input itself is chosen in the Record Audio dialog,
  not here.

<!-- Image placeholder: the Preferences Audio tab showing the Output device list and the Recording input driver dropdown. -->

## MIDI

Set up supported [MIDI Controllers](/1.9/guide/midi-controllers):
...
[Brake and Backspin](/1.9/guide/clip-editor#turntable-effects-brake-and-backspin)):
...
These settings shape the [Scratch Editor](/1.9/guide/scratch-editor):
...
Defaults for [stem separation](/1.9/guide/stems). *Models* are the engines Silverdaw
