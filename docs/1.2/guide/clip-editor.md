...
To record a vinyl-style pattern instead, right-click a clip and choose **Open in
Scratch Editor**. See [Scratch Editor](/1.2/guide/scratch-editor).
...
- **A [linked clip](/1.2/guide/library#linked-clips)** — or a **saved clip** opened
...
choose **Unlink from Library** first (see [linked clips](/1.2/guide/library#linked-clips)).

![The Clip Editor window](/1.2/images/clip-editor.png)
...
- **Editing a clip.** Double-click a clip's body on the timeline, or right-click
  ...
  **Save**.
- **Previewing a file.** Double-click a [Library](/1.2/guide/library) tile (anywhere
...
- **Metronome** — the metronome button in the transport (or the <kbd>K</kbd> key)
  ...
  [timeline metronome](/1.2/guide/timeline#the-metronome), and it isn't shown when
...
new saved clip you can reuse — the same [saved clips](/1.2/guide/library#saved-clips-and-samples)
...
- **Tempo** — type the correct **BPM** and press <kbd>Enter</kbd>. If the tempo
  ...
  one click, and **Restore** puts back the originally detected tempo. You can also
...
1. [Split](/1.2/guide/timeline#moving-and-editing-clips) or trim the clip down to a
   short stretch with a clear, steady beat.
2. Right-click it and choose **Save as Sample (Music)** to bake that section into
   a new [music sample](/1.2/guide/library#music-and-simple-samples) in the Library.
3. Right-click the new sample in the [Library](/1.2/guide/library) and choose
...
- **Slice to samples** — save each slice as its own sample in the
  [Library](/1.2/guide/library) to reuse and rebuild.
