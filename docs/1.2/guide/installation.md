...
- **Graphics card (optional):** a compatible GPU can speed up
  [stem separation](/1.2/guide/stems), but it is not required — Silverdaw can do the
...
- **MIDI DJ controller (optional):** connect a supported controller after
  installation, then set it up in [MIDI Controllers](/1.2/guide/midi-controllers).
...
[Quick Start](/1.2/guide/quick-start) to build your first remix, or take the
[tour of the Silverdaw window](/1.2/guide/the-window) first.

> If you do not hear anything when you press play, check that the right audio
> output is selected. See [Choosing your audio output](/1.2/guide/the-window#choosing-your-audio-output).
