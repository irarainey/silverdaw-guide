...
controls in [Mixing & Effects](/1.7/guide/mixing) work on frequency, turning
...
- Each project has one **Sample rate**, set in **File ▸ Project Properties…**.
  See [Projects](/1.7/guide/projects#project-properties).
- New projects use the **Default project sample rate** from
  [Preferences ▸ Audio](/1.7/guide/preferences#audio).
- If you import a file recorded at a different rate, Silverdaw asks whether to
  **Convert to** the project's rate or **Switch Project to** the file's rate.
  See [Importing & the Library](/1.7/guide/library#sample-rate-check).
...
[export a mixdown](/1.7/guide/export). Export defaults to **16-bit**, with
...
The **Bit Crusher** in [Track FX](/1.7/guide/mixing#track-fx) uses the same
...
- **Transform ▸ Split Stereo Channels…** splits a stereo clip's **Left** and
  **Right** sides onto their own clips, which is useful when the two sides carry
  different material. See [Arranging the Timeline](/1.7/guide/timeline).
- Clips can be drawn as a **Single waveform** or as **Left and right** lanes —
  set **Waveform display** in
  [Preferences ▸ General](/1.7/guide/preferences#general). Mono clips always
...
Two more terms appear in the [Export Mixdown](/1.7/guide/export) dialog, where
...
- [Importing & the Library](/1.7/guide/library) — getting audio into a project.
- [Mixing & Effects](/1.7/guide/mixing) — shaping levels and tone.
- [Exporting a Mixdown](/1.7/guide/export) — turning the project into a file.
